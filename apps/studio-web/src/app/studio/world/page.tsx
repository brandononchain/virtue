...
export default function WorldPage() {
  ...
  const TABS: { key: ActiveTab; label: string }[] = [
    ...
    { key: "events", label: "Story Events" },
    { key: "graph", label: "World Graph" },
  ];
    ...
    <div className="p-8 space-y-6 overflow-y-auto">
        ...
        <div>
          ...
          <div className="flex gap-2">
            <select
              ...
              className="flex-1 rounded-md border border-zinc-800 bg-zinc-900 px-3 py-2 text-sm text-zinc-200 focus:border-zinc-600 focus:outline-none disabled:opacity-40"
            >
            ...
            <button
              ...
              className="rounded-md bg-purple-900/40 border border-purple-700/50 px-3 py-2 text-xs text-purple-300 hover:bg-purple-900/60 disabled:opacity-40 transition-colors whitespace-nowrap"
            >
      ...
      {loading && (
        <div className="studio-panel p-6 text-center">
          <p className="text-xs text-zinc-500 animate-pulse">Loading world state...</p>
        </div>
      )}
      ...
      {world && !loading && (
        <div className="grid grid-cols-5 gap-3">
          ...
          <StatCard label="Environments" value={world.environments.length} />
          <StatCard label="Props" value={world.props.length} />
          <StatCard label="Events" value={world.storyEvents.length} />
          <StatCard label="Timeline" value={world.timelinePosition} />
        </div>
      )}
        ...
        <div className="space-y-2">
          {world.characters.length === 0 ? (
            ...
            world.characters.map((cs) => (
              <div key={cs.characterId} className="studio-panel p-4">
                <div className="flex items-center gap-3 mb-2">
                  ...
                  <div className="flex-1">
                    <p className="text-sm text-zinc-200 font-medium">{charName(cs.characterId)}</p>
                    <p className="text-[10px] text-zinc-600 font-mono">{cs.characterId}</p>
                  </div>
                  <button
                    onClick={() => {
                    ...
                    className="text-[10px] text-zinc-600 hover:text-zinc-300 transition-colors uppercase"
                  >
                ...
                {Object.keys(cs.relationships).length > 0 && (
                  <div className="mt-2 flex gap-2 flex-wrap">
                    {Object.entries(cs.relationships).map(([id, rel]) => (
                      <span key={id} className="rounded bg-zinc-800 px-2 py-0.5 text-[9px] text-zinc-400">
                        {charName(id)}: {rel}
                      </span>
                    ))}
                  </div>
                ...
                {editingChar === cs.characterId && (
                  <div className="mt-3 pt-3 border-t border-zinc-800/40 grid grid-cols-3 gap-2">
                    <div>
                      <label className="block text-[9px] text-zinc-600 uppercase mb-1">Emotion</label>
                      <input type="text" value={editEmotion} onChange={(e) => setEditEmotion(e.target.value)}
                        className="w-full rounded border border-zinc-800 bg-zinc-900 px-2 py-1 text-xs text-zinc-200 focus:outline-none focus:border-zinc-600" />
                    </div>
                    <div>
                      <label className="block text-[9px] text-zinc-600 uppercase mb-1">Condition</label>
                      <input type="text" value={editCondition} onChange={(e) => setEditCondition(e.target.value)}
                        className="w-full rounded border border-zinc-800 bg-zinc-900 px-2 py-1 text-xs text-zinc-200 focus:outline-none focus:border-zinc-600" />
                    </div>
                    <div>
                      <label className="block text-[9px] text-zinc-600 uppercase mb-1">Location</label>
                      <input type="text" value={editLocation} onChange={(e) => setEditLocation(e.target.value)}
                        className="w-full rounded border border-zinc-800 bg-zinc-900 px-2 py-1 text-xs text-zinc-200 focus:outline-none focus:border-zinc-600" />
                    </div>
                    <div className="col-span-3 flex justify-end">
                      <button onClick={() => handleUpdateCharacter(cs.characterId)}
                        className="rounded bg-zinc-100 px-3 py-1 text-xs font-medium text-zinc-900 hover:bg-white transition-colors">
                        Save
                      </button>
                    </div>
                  </div>
        ...
        <div className="space-y-2">
          {world.environments.length === 0 ? (
            ...
            world.environments.map((es) => (
              <div key={es.environmentId} className="studio-panel p-4">
                <div className="flex items-center gap-3 mb-2">
                  ...
                  <div>
                    <p className="text-sm text-zinc-200 font-medium">{envName(es.environmentId)}</p>
                    <p className="text-[10px] text-zinc-600 font-mono">{es.environmentId}</p>
                  </div>
                </div>
                <div className="grid grid-cols-5 gap-3">
      ...
      {world && activeTab === "props" && (
        <div className="space-y-2">
          {world.props.length === 0 ? (
            ...
            world.props.map((ps) => (
              <div key={ps.propId} className="studio-panel p-3 flex items-center gap-3">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-xs text-zinc-200 font-medium">{propName(ps.propId)}</p>
                  <p className="text-[10px] text-zinc-600 font-mono">{ps.propId}</p>
                </div>
                ...
                <span className={`rounded px-1.5 py-0.5 text-[8px] uppercase font-medium ${
                  ...
                  : ps.visibility === "hidden" ? "bg-zinc-800 text-zinc-500"
        ...
        <div className="space-y-2">
          {world.storyEvents.length === 0 ? (
            ...
            world.storyEvents.map((evt, i) => (
              <div key={evt.id} className="studio-panel p-3 flex items-start gap-3">
                <span className="text-[10px] text-zinc-700 font-mono w-6 shrink-0 pt-0.5">
        ...
        <div className="studio-panel p-6">
          <div className="flex items-center justify-center min-h-[400px]">
            <div className="space-y-6 w-full max-w-3xl">
              {/* Characters row */}
              <div>
                <label className="block text-[9px] text-zinc-600 uppercase tracking-wider mb-2">Characters</label>
                    ...
                    <div key={cs.characterId} className="rounded-lg bg-blue-950/20 border border-blue-900/30 px-3 py-2 min-w-[120px]">
                      ...
                      {cs.location !== "unknown" && (
                        <div className="flex items-center gap-1 mt-1">
                          <span className="text-[8px] text-zinc-600">at</span>
              ...
              {/* Environments row */}
              <div>
                <label className="block text-[9px] text-zinc-600 uppercase tracking-wider mb-2">Environments</label>
                <div className="flex gap-3 flex-wrap">
                  {world.environments.map((es) => (
              ...
              {world.props.length > 0 && (
                <>
                  ...
                  <div>
                    <label className="block text-[9px] text-zinc-600 uppercase tracking-wider mb-2">Props</label>
                    <div className="flex gap-3 flex-wrap">
                      {world.props.map((ps) => (
                        <div key={ps.propId} className={`rounded-lg border px-3 py-2 min-w-[100px] ${
                          ...
                          : ps.visibility === "hidden" ? "bg-zinc-900/30 border-zinc-800/30 opacity-50"
              ...
              <div className="border-t border-zinc-800/40 pt-4 mt-4">
                <label className="block text-[9px] text-zinc-600 uppercase tracking-wider mb-2">
                ...
                <div className="flex gap-1">
                  ...
                  {world.storyEvents.length === 0 && (
                    <p className="text-[10px] text-zinc-700 italic">No events yet</p>
                  )}
                </div>
              </div>
            </div>
      ...
      {!selectedProject && !loading && (
        <div className="studio-panel p-12 text-center">
          <p className="text-sm text-zinc-600">Select a project to view its world state.</p>
        </div>
      )}
    </div>
  );
...
function StateField({
  ...
  if (inline) {
    return (
      <div className="shrink-0">
        <p className="text-[8px] text-zinc-700 uppercase">{label}</p>
        <p className={`text-[10px] font-mono ${
          highlight ? (warn ? "text-red-400" : "text-cyan-400") : "text-zinc-400"
        }`}>{value}</p>
      </div>
    );
  }
  return (
    <div>
      <p className="text-[9px] text-zinc-600 uppercase mb-0.5">{label}</p>
      <p className={`text-xs font-mono ${
        highlight ? (warn ? "text-red-400" : "text-cyan-400") : "text-zinc-300"
      }`}>{value}</p>
    </div>
  );
}
